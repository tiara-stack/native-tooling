...
Welcome to your guide on testing Effect-based applications using Vite+ and the `@tiara-stack/effect-vitest` package. This package is derived from `@tiara-stack/effect-vitest` and routes Vitest imports through Vite+'s test exports so workspaces use the same test runner instance.

In this guide, we'll walk you through setting up the necessary dependencies and provide examples of how to write Effect-based tests using `@tiara-stack/effect-vitest`.
...
```sh
pnpm add -D @tiara-stack/effect-vitest
```
...
```ts
import { it } from "@tiara-stack/effect-vitest";
```
...
```ts
import { expect, it } from "@tiara-stack/effect-vitest";
...
```ts
import { it } from "@tiara-stack/effect-vitest";
import { expect } from "@tiara-stack/effect-vitest";
import { Effect, Exit } from "effect";
